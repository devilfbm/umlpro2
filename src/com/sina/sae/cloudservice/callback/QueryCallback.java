...
public abstract class QueryCallback  implements ActionCallback{
	...
	public void done(JsonObject returnValue, CloudServiceException e) {
		...
		if(e == null){
			...
			if(0==code&&"success".equalsIgnoreCase(message)){
				...
				Gson gson = new Gson();
				list =  gson.fromJson(data,new TypeToken<List<Map<String,String>>>(){}.getType());  
			}else{
				String errorMessage = "QueryCallback.done("+returnValue+","+e+") Error!Code: "+ code+" message:"+message;
	 ...
	 * @param list 云端数据库查询到的数据并转换成相应对象list
	 * @param e 操作过程中出现的异常
	 */
