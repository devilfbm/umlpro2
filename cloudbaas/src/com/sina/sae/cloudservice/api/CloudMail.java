...
public class CloudMail {
	...
	public boolean send()throws CloudServiceException{
		...
		if(0==code&&"success".equalsIgnoreCase(message)){
			...
			String errorMessage = "CloudMail.send() Error!Params:"+toString()+"  Code: "+ code+" message:"+message;
	...
	private Map<String,String>  getParams(){
		//验证必填参数
		if(null==from||null==smtpusername||null==smtpPassword||null==to||null==subject||null==content||null==smtpHost){
			return null;
		}
		...
		if(chartset!=null)params.put("chartset", chartset);
		params.put("tls", tls+"");
		return params;
	}
	...
	/**
	 * 设置邮件正文展现方式 
	 * @param contentType 邮件正文展现方式 "TEXT"|"HTML" 默认为TEXT
	 */
	...
	public String toString() {
		return "CloudMail [from=" + from + ", smtpusername=" + smtpusername
				+ ", smtpPassword=" + smtpPassword + ", cc="
				+ Arrays.toString(cc) + ", to=" + Arrays.toString(to)
				+ ", subject=" + subject + ", content=" + content + ",smtpPort=" + smtpPort
				+ ", contentType=" + contentType + ", smtpHost=" + smtpHost
				+ ", chartset=" + chartset + ", tls=" + tls + "]";
	}
